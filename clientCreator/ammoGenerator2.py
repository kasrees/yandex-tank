...
def make_ammo(method, url, headers, case, body):
    ...
    return ammo_template % (len(req) + 6, case, req)
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        sys.stdout.write("[Host: bookingreport-qa.travelline.lan:443]\n")

        for provider in providers:
            property_id = str(provider).strip()
            url = f"/BookingReport/WebApi/PartnerApi/GetBookings?ProviderId={property_id}&LastModifiedMillisecondsFrom=1230757200000&LastModifiedMillisecondsTo=1262293200000&WithoutDate=True&limit=10\n"
